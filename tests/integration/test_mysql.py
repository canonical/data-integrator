...
async def test_deploy_and_relate_mysql(ops_test: OpsTest):
    ...
    integrator_relation = await ops_test.model.add_relation(
        DATA_INTEGRATOR, MYSQL[ops_test.cloud_name]
    )
...
async def test_deploy_and_relate_mysql_router(ops_test: OpsTest):
    ...
    channel = "dpe/edge" if ops_test.cloud_name == "localhost" else "8.0/edge"
    await asyncio.gather(
        ops_test.model.deploy(
    ...
    await ops_test.model.wait_for_idle(
        apps=[DATA_INTEGRATOR, MYSQL[ops_test.cloud_name], MYSQL_ROUTER[ops_test.cloud_name]],
        status="active",
    )
    assert ops_test.model.applications[DATA_INTEGRATOR].status == "active"
