...
class Database:
    ...
    def create_table(self, name: str, schema: list[tuple[str, SchemaType]]):
        ...
        with self.client.connection as conn, conn.cursor() as cursor:
            schema_str = ", ".join(f"{col_name} {TYPES[_type]}" for col_name, _type in schema)
...
class Table:
    def __init__(self, name: str, database: Database, schema: list[tuple[str, SchemaType]]):
        self.name = name
    ...
    def validate(self, values: list[SchemaType]):
        ...
        for value, (col_name, _type) in zip(values, self.schema):
            if not isinstance(value, _type):
                raise TypeError(f"Column {col_name}: Expected {_type}, found {type(value)}")
